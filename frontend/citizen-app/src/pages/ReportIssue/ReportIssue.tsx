...
function ReportIssue() {
  ...
  // ── Step 2 → 3: geocode only if no map pin was dropped ───────────────────
  const handleStep2Continue = async () => {
    ...
    try {
      if (latitude === null || longitude === null) {
        ...
        if (!found) {
          setError(
            "Could not find coordinates for the entered location. " +
            "Try being more specific, or drop a pin on the map."
          );
          return;
        }
      }
      ...
      <div className="ri-success-page">
        <div className="ri-success-card">
          ...
          <div className="ri-success-actions">
            <button className="ri-btn-outline" onClick={() => navigate("/issues")}>
              View All Issues
            </button>
            <button className="ri-btn-primary" onClick={resetForm}>
              Report Another
            </button>
          </div>
        </div>
  ...
  return (
    <div className="ri-page">
      ...
      <footer className="ri-footer">
        {step > 1 ? (
          <button className="ri-btn-ghost" onClick={() => { setError(""); setStep(step - 1); }}>
            ← Back
          </button>
        ) : (
          <button className="ri-btn-ghost" onClick={() => navigate("/")}>
            Cancel
          </button>
        )}
        ...
        {step < 3 ? (
          ...
          >
            {loading ? (
              <><span className="ri-spinner" /> Submitting...</>
            ) : (
              "Submit Report →"
            )}
          </button>
